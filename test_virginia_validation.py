...
logger = logging.getLogger("Virginia_Validator")

def run_test():
    logger.info("Initializing Virginia FARM VEHICLE OCR Integration Test...")
    ...
    test_img_name = "96e66642-00d5963c8f8e5381_jpg.rf.294ff94b8b93ed179203559c25fdcbe0.jpg"
    test_img_path = os.path.join("dataset", "images", "train", test_img_name)
    
    if not os.path.exists(test_img_path):
        logger.error(f"Test image '{test_img_name}' not found!")
        sys.exit(1)
    ...
    for det in detections:
        ...
        if det['class'] == 'Number plate':
            ...
            if easy_text == "F 25-334" and tess_text == "F 25-334":
                logger.info("SUCCESS: Both EasyOCR and Tesseract OCR successfully resolved to 'F 25-334'!")
                passed = True
            else:
                logger.error(f"FAILURE: OCR text mismatch. Expected 'F 25-334', got EasyOCR='{easy_text}', Tesseract='{tess_text}'")
                
    if passed:
        logger.info("VIRGINIA PLATE INTEGRATION TEST PASSED 100%!")
        sys.exit(0)
    else:
        logger.error("VIRGINIA PLATE INTEGRATION TEST FAILED!")
        sys.exit(1)
